...
//初期設定関数.
void clsSound::SetInitParam( const std::string& sAlias, HWND hWnd )
{
	//ｳｨﾝﾄﾞｳﾊﾝﾄﾞﾙを設定.
	m_hWnd = hWnd;

	//ｴｲﾘｱｽを設定.
	strcpy_s( m_sAlias, sizeof( m_sAlias ), sAlias.c_str() );
}

//音声ﾌｧｲﾙを開く.
bool clsSound::Open(const LPCTSTR& sFName,const std::string& sAlias, HWND hWnd)
{
	//初期設定.
...
//音量を調整する関数.
bool clsSound::VolumeConvert(const int& volume)
{
	//コマンド.
