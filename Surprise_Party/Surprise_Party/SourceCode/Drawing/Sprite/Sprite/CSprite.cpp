...
void CSprite::Render(const D3DXMATRIX& mView, const D3DXMATRIX& mProj, const D3DXVECTOR3& vCamPos)
{
			...
			0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
	{
		...
		//カラー.
		cb.vColor = D3DXVECTOR4(m_vColor.x, m_vColor.y, m_vColor.z, m_fAlpha);

		//UV座標.
		//１コマ当たりの割合にﾊﾟﾀｰﾝ番号をかけて位置を設定する.
		cb.vUV.x = (m_SState.Stride.w / m_SState.Base.w) * m_vPatternNo.x;
		cb.vUV.y = (m_SState.Stride.h / m_SState.Base.h) * m_vPatternNo.y;
		cb.vUV.w = 0.0f;
		cb.vUV.z = 0.0f;
	...
	m_pContext11->VSSetConstantBuffers(3, 1, &m_pConstantBuffer);
	m_pContext11->PSSetConstantBuffers(3, 1, &m_pConstantBuffer);
